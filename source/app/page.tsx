...
declare global {
  interface Window {
    DLT_HISTORY_DATA?: Record<string, DashboardData>;
  }
}
...
type DataSource = "fixed-history" | "top-ranking";
type DashboardData = { issue: string; experts: Expert[]; killers: Killer[]; updatedAt?: string; sourceType?: DataSource };

const initialData: DashboardData = { issue: "2026085", experts, killers, sourceType: "top-ranking" };
const staticHistory = window.DLT_HISTORY_DATA ?? {};

export default function Home() {
  const [data, setData] = useState<DashboardData>(staticHistory["2026085"] ?? initialData);
  const [history, setHistory] = useState<Record<string, DashboardData>>(staticHistory);
  const [issueInput, setIssueInput] = useState("2026085");
  const [historyMessage, setHistoryMessage] = useState("输入期数可快速查看已保存快照");
  ...
  const availableIssues = useMemo(
    () => Object.keys(history).sort((a, b) => Number(a) - Number(b)),
    [history],
  );
  const issueIndex = availableIssues.indexOf(data.issue);

  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("dlt-history-snapshots") ?? "{}") as Record<string, DashboardData>;
      setHistory({ ...staticHistory, ...saved });
    } catch {
      setHistory(staticHistory);
    }
  }, []);

  const applySnapshot = (snapshot: DashboardData) => {
    setData(snapshot);
    setIssueInput(snapshot.issue);
    setSelected(0);
    setSelectedExperts(snapshot.experts.map((expert) => expert.name));
    setSelectedKillers(snapshot.killers.map((killer) => killer.name));
    setViewMode("single");
    setFilter("all");
  };

  const openIssue = (issue: string) => {
    if (!/^2026\d{3}$/.test(issue) || Number(issue.slice(4)) < 1) {
      setHistoryMessage("请输入正确期数，例如 2026001");
      return;
    }
    const snapshot = history[issue];
    if (!snapshot) {
      setHistoryMessage(`${issue} 尚未写入静态历史库`);
      return;
    }
    applySnapshot(snapshot);
    setHistoryMessage(`已切换至 ${issue} 期`);
  };

  const moveIssue = (offset: number) => {
    const next = availableIssues[issueIndex + offset];
    if (next) openIssue(next);
  };
  ...
  const openPasswordDialog = () => {
    ...
    setRefreshMessage("离线版不支持联网刷新，请更新 assets/history-data.js 后重新打开页面");
  };
  ...
  const refreshData = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const nextHistory = { ...history, [payload.issue]: payload };
      setHistory(nextHistory);
      localStorage.setItem("dlt-history-snapshots", JSON.stringify(
        Object.fromEntries(Object.entries(nextHistory).filter(([issue]) => !staticHistory[issue])),
      ));
      ...
      <div className="page-shell">
        ...
        <section className="history-toolbar" aria-labelledby="history-title">
          <div className="history-copy">
            <p className="panel-kicker">HISTORICAL SNAPSHOTS</p>
            <h2 id="history-title">往期专家杀号对照</h2>
            <p>历史数据从2026001期起；输入期数后，整页专家方案、矩阵和统计同步切换。</p>
          </div>
          <form
            className="issue-search"
            onSubmit={(event) => {
              event.preventDefault();
              openIssue(issueInput);
            }}
          >
            <label htmlFor="issue-input">输入期数</label>
            <div>
              <button type="button" aria-label="上一期" disabled={issueIndex <= 0} onClick={() => moveIssue(-1)}>←</button>
              <input
                id="issue-input"
                inputMode="numeric"
                maxLength={7}
                value={issueInput}
                onChange={(event) => setIssueInput(event.target.value.replace(/\D/g, "").slice(0, 7))}
                placeholder="例如 2026001"
              />
              <button type="submit">查看</button>
              <button type="button" aria-label="下一期" disabled={issueIndex < 0 || issueIndex >= availableIssues.length - 1} onClick={() => moveIssue(1)}>→</button>
            </div>
            <small aria-live="polite">{historyMessage}</small>
          </form>
          <div
            className={`source-badge ${data.sourceType === "fixed-history" ? "fixed" : "top"}`}
            aria-label={`本期数据口径：${data.sourceType === "fixed-history" ? "历史固定专家数据" : "当期 Top 榜单数据"}，已保存 ${availableIssues.length} 期快照`}
          >
            <span>本期数据口径</span>
            <strong>{data.sourceType === "fixed-history" ? "历史固定专家数据" : "当期 Top 榜单数据"}</strong>
            <small>已保存 {availableIssues.length} 期快照</small>
          </div>
        </section>
